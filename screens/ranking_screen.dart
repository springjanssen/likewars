...
Widget _buildFameRanking(String? currentUserId) {
          ...
          return ListTile(
            leading: CircleAvatar(
              backgroundImage: player.photoURL != null && player.photoURL.isNotEmpty
                  ...
                  : null, // Show nothing if no URL
              backgroundColor: player.photoURL == null || player.photoURL.isEmpty
                  ? Colors.grey // Default color if no photo
                  : Colors.transparent, // No background if there's an image
            ),
            title: Text(
              player.displayName,
            ...
            trailing: isCurrentUser
                ? Icon(Icons.star, color: Colors.yellow) // Star icon for the current user
                : Text('#${index + 1}'), // Rank for others
          );
        },
      );
    },
  ...
  Widget _buildChallengeLeaderboardTable(
    ...
    final totalPlayers = sortedSubmissions.length.toDouble();
    ...
    int _calculateFame(int rank, double totalPlayers) {
      if (rank <= 0 || totalPlayers <= 0) return 0;
      return (100 * sqrt(totalPlayers) * (1 / rank) * (1 + (totalPlayers / 1000))).round();
    }
